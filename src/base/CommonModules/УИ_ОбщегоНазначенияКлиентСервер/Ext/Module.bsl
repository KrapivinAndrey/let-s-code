...
Функция СогласоватьСЧислительными(Число, ПараметрыПрописи) Экспорт
	
	Варианты = СтрРазделить(ПараметрыПрописи, ",");
	Если Варианты.Количество() <> 3 Тогда
		
		ВызватьИсключение "Должно быть указано три варианта прописи " + ПараметрыПрописи;
	...
	Если (ОднаЦифра >= 2 И ОднаЦифра <= 4) 
			И Не (ДвеЦифры >= 12 И ДвеЦифры <= 14) Тогда
		
		Суффикс = Варианты[1];
		
	ИначеЕсли ОднаЦифра = 1 И Не ДвеЦифры = 11 Тогда
	...
	ЧислоПрописью = Формат(Число, "ЧН=; ЧГ=");

	Возврат ЧислоПрописью + " " + СокрЛП(Суффикс);
